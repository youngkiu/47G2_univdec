...
DWLLinearMem_t sProbTbl = { 0, };
DWLLinearMem_t sSegmentMap = { 0, };
...
void VP8HwdAsicInit(VP8DecContainer_t * pDecCont)
{
	...
	if(sSegmentMap.busAddress == 0)
	{
		dwl_ret = DWLMallocLinear(NULL, 2048, &sSegmentMap);
		if(dwl_ret != DWL_OK)
		{
			return;
		}
...
i32 VP8HwdAsicAllocateMem(VP8DecContainer_t * pDecCont)
{
	...
	if(sProbTbl.busAddress == 0)
	{
		dwl_ret = DWLMallocLinear(dwl, PROB_TABLE_SIZE, &sProbTbl);
		if(dwl_ret != DWL_OK)
		{
	...
	pAsicBuff->probTbl = sProbTbl;
	SetDecRegister(pDecCont->vp8Regs, HWIF_VP6HWPROBTBL_BASE, pAsicBuff->probTbl.busAddress);
...
i32 VP8HwdAsicAllocatePictures(VP8DecContainer_t * pDecCont)
{
	...
	u32 memorySize;
#if 0
	u32 ui32BufStride = (pAsicBuff->width + 0xF) & (~0xF);
	u32 ui32BufHeight = (pAsicBuff->height + 0xF) & (~0xF);
	u32 ui32offset = (ui32BufStride*ui32BufHeight)&0x3FFF;

	if(ui32offset!=0)
	{
		ui32offset = 0x4000 - ui32offset;
		SetDecRegister(pDecCont->vp8Regs, HWIF_OUT_BASE_MODE, 0x1);
		SetDecRegister(pDecCont->vp8Regs, HWIF_OUT_YC_OFFSET, ui32offset);
	}
#endif
	/* allocate segment map */

	// MAx Size :: (((120*68 + 3)/4)+63)/64*64 = 2048
	...
	pAsicBuff->segmentMap = sSegmentMap;

	pAsicBuff->segmentMapSize = memorySize;
	SetDecRegister(pDecCont->vp8Regs, HWIF_SEGMENT_BASE, pAsicBuff->segmentMap.busAddress);
	DWLmemset(pAsicBuff->segmentMap.virtualAddress, 0, pAsicBuff->segmentMapSize);
	...
	if (!pDecCont->sliceHeight)
		...
		pict_buff_size = pAsicBuff->width * (pDecCont->sliceHeight + 1) * 16 * 3 / 2;

	dwl_ret = BqueueInit(&pDecCont->bq, pDecCont->numBuffers );
	...
	{
		pDecCont->pp.ppInfo.tiledMode = pDecCont->tiledReferenceEnable;
		pDecCont->pp.PPConfigQuery(pDecCont->pp.ppInstance, &pDecCont->pp.ppInfo);
	}
	if (!pDecCont->userMem &&
	    !(pDecCont->intraOnly && pDecCont->pp.ppInfo.pipelineAccepted))
	{
		...
		{
			SetDecRegister(pDecCont->vp8Regs, HWIF_OUT_BASE_MODE, 0x1);
			SetDecRegister(pDecCont->vp8Regs, HWIF_OUT_YC_OFFSET, (pAsicBuff->pictures[0].size - pict_buff_size) * 2 / 3);
		}
	}
	...
	if (pDecCont->intraOnly)
	{
		...
		* of mbs stored/load per access */
		...
		SetDecRegister(pDecCont->vp8Regs, HWIF_REFER6_BASE, pAsicBuff->pictures[1].busAddress);
		SetDecRegister(pDecCont->vp8Regs, HWIF_REFER2_BASE, pAsicBuff->pictures[1].busAddress + 4*tmpW);
		SetDecRegister(pDecCont->vp8Regs, HWIF_REFER3_BASE, pAsicBuff->pictures[1].busAddress + 36*tmpW);
	}
