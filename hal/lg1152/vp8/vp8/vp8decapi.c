...
#include "../inc/decapicommon.h"
#include "../inc/vp8decapi.h"
...
#include "../common/regdrv.h"
#include "../common/refbuffer.h"
...
#ifdef VP8DEC_TRACE
#define DEC_API_TRC(str)    VDEC_KDRV_Message(DBG_VDC, "[VP8DEC] "str)
#else
...
static VP8DecContainer_t sDecInstance[NUM_OF_VP8_CHANNEL];
...
------------------------------------------------------------------------------*/
VP8DecRet VP8DecInit(u8 ui8ch, VP8DecInst * pDecInst, VP8DecFormat decFormat,
                     u32 useVideoFreezeConcealment,
                     u32 numFrameBuffers,
...
#error Right bit-shifting (>>) does not preserve the sign
	...
	*pDecInst = NULL;   /* return NULL instance for any error */
	...
	/* check that decoding supported in HW */
	{
		...
		if((decFormat == VP8DEC_VP8 || decFormat == VP8DEC_WEBP) && !hwCfg.vp8Support)
		{
                       ...
                       const VP8DecInput * pInput, VP8DecOutput * pOutput)
{
	...
	{
		DEC_API_TRC("VP8DecDecode# ERROR: Invalid AUI arg value\n");
		return VP8DEC_PARAM_ERROR;
	}
	...
	if ((pInput->pPicBufferY != NULL && pInput->picBufferBusAddressY == 0) ||
		...
		(pInput->pPicBufferY != NULL && pInput->pPicBufferC == 0))
	{
		DEC_API_TRC("VP8DecDecode# ERROR: Invalid Pic. arg value\n");
		return VP8DEC_PARAM_ERROR;
	}
	...
	* disabled unless WebP and PP not used */
	...
	if (pDecCont->decStat == VP8DEC_NEW_HEADERS)
	{
		...
		{
			DEC_API_TRC("VP8DecDecode# ERROR: Picture memory allocation failed\n");
			return VP8DEC_MEMFAIL;
		}
	}
	else if (pDecCont->decStat != VP8DEC_MIDDLE_OF_PIC)
	{
		...
		else if(pOutput->picScanMode!=0 || pOutput->findKeyFrame==0)		// sooya
		{
			return VP8DEC_STRM_ERROR;
		}
		...
		ret = vp8hwdDecodeFrameHeader(
					(u8 *)(pInput->pStream + pDecCont->decoder.frameTagSize),
		...
		ret = vp8hwdSetPartitionOffsets((u8 *)pInput->pStream, pInput->dataLen, &pDecCont->decoder);
		if (ret != HANTRO_OK)
		{
			if (!pDecCont->picNumber || pDecCont->decStat != VP8DEC_DECODING)
				return VP8DEC_STRM_ERROR;
		...
		(asic_status & DEC_8190_IRQ_ERROR))
	{
		...
		if(pDecCont->pp.ppInstance != NULL && pDecCont->pp.decPpIf.usePipeline)
		{
			...
			TRACE_PP_CTRL("VP8DecDecode: Concealed picture, PP should run again\n");
...
//		i32 i;
	...
	else
	{
		...
		{
			(void) DWLmemset( pAsicBuff->refBuffer->virtualAddress, 128, pAsicBuff->width * pAsicBuff->height * 3 / 2);
		}
	}
                            ...
                            VP8DecPicture * pOutput, u32 endOfStream)
{
	...
	{
		VDEC_KDRV_Message(DBG_VDC, "[VP8][DBG] VP8DEC_OK %s", __FUNCTION__);
		return VP8DEC_OK;
	}
...
//			u32 offset = 16 * (pDecCont->sliceHeight + 1) * pAsicBuff->width;
			u32 offset = pAsicBuff->pictures[0].size * 2 / 3;

			pOutput->pOutputFrame = pAsicBuff->pictures[0].virtualAddress;
			pOutput->outputFrameBusAddress = (u32)pAsicBuff->pictures[0].virtualAddress;

			pOutput->pOutputFrameC = (u32 *)(pOutput->pOutputFrame + offset);
			pOutput->outputFrameBusAddressC = pOutput->outputFrameBusAddress + offset;
		}
		...
		VDEC_KDRV_Message(DBG_VDC, "[VP8][DBG] VP8DEC_PIC_RDY 1 %s", __FUNCTION__);
...
//			pOutput->pOutputFrameC = pOutput->pOutputFrame + pAsicBuff->width * pAsicBuff->height / 4;
//			pOutput->outputFrameBusAddressC = pOutput->outputFrameBusAddress + pAsicBuff->width * pAsicBuff->height;
			pOutput->pOutputFrameC = pOutput->pOutputFrame + (outPic->size * 2 / 3) / 4;
			pOutput->outputFrameBusAddressC = pOutput->outputFrameBusAddress + (outPic->size * 2 / 3);
		}
		pOutput->picId = 0;
		pOutput->isIntraFrame = pDecCont->decoder.keyFrame;
		pOutput->isGoldenFrame = 0;
		...
		VDEC_KDRV_Message(DBG_VDC, "[VP8][DBG] VP8DEC_PIC_RDY 2 %s", __FUNCTION__);
		return (VP8DEC_PIC_RDY);
	}
