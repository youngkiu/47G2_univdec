...
void IPC_CMD_Init(void)
{
	...
	ui32MemPtr = IPC_REG_CMD_GetBaseAddr();
	ui32MemSize = IPC_REG_CMD_GetBufSize();

	for( i = 0; i < IPC_CMD_ID_MAX; i++ )
	{
		fpIpcCmd_Receiver[i] = NULL;
	}
#else
	...
	TOP_HAL_ClearInterIntr(LQ0_DONE+IPC_ISR_ID);
	TOP_HAL_EnableInterIntr(LQ0_DONE+IPC_ISR_ID);
#else
//	TOP_HAL_ClearExtIntr(LQ0_DONE+IPC_ISR_ID);
//	TOP_HAL_EnableExtIntr(LQ0_DONE+IPC_ISR_ID);
#endif
}
...
********************************************************************************
*/
static BOOLEAN _IPC_CMD_IsWithinRange(UINT32 ui32StartAddr, UINT32 ui32EndAddr, UINT32 ui32TargetAddr)
{
...
static UINT32 _IPC_CMD_CheckOverflow(UINT32 ui32CmdSize)
{
	...
	if( _IPC_CMD_IsWithinRange(ui32WrOffset_Org, ui32WrOffset + ui32CmdSize, ui32RdOffset) == TRUE )
	{
